            ...
            {initialLoading ? (
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {clubs.map((club: Club) => {
                    // Check if club has positions
                    if (!club.positions || !Array.isArray(club.positions) || club.positions.length === 0) {
                      return null;
                    }
                    
                    // Return a PositionCard for each position in the club
                    return club.positions.map((position: any, index: number) => (
                      <PositionCard
                        key={`${club.id}-${index}`}
                        position={position}
                        club={club}
